...
def create_ssdlite(pretrained: bool = False, progress: bool = True, num_classes: int = 91,
    ...
    # GaugeNet returns default value: trainable_backbone_layers = 6
    trainable_backbone_layers = _validate_trainable_layers(pretrained or pretrained_backbone, trainable_backbone_layers, 6, 6)
    ...
    size = (320, 320)
    anchor_generator = DefaultBoxGenerator([[2, 3] for _ in range(6)], min_ratio=0.2, max_ratio=0.95) # MOD
    out_channels = det_utils.retrieve_out_channels(backbone, size)
    num_anchors = anchor_generator.num_anchors_per_location() # MOD 
...
class SSDLiteRegressionHead(SSDScoringHead):  # MOD
